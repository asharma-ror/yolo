...
start sniffer by simply calling it
"""
...
class Options:
    ...
    targets = [
        # (
        #     ("dmypy", "run", "--", "--follow-imports=error", "server",),
        #     "Faster Type checks",
        #     True,
        # ),
        # (("inv", "flake8"), "Linter each file alone", False,),
        (("flake8", "server", "--count"), "Linter checks", False,),
        # (("pylint", "server",), "Linter checks", False,),
        (("mypy", "server"), "Type checks", True,),
        (("pytest", "-x", "--failed-first", "--disable-warnings"), "Unit Tests", True),
    ]
